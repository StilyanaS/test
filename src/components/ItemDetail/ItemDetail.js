...
const ItemDetail = ({ item }) => {
  ...
  return (
    <Row className="mt-3">
      <Col>
        <Image className="product-img" src={item.image} rounded />
      </Col>
      <Col>
        <Card style={{ margin: "1rem" }} className="card">
          <Card.Header as="h1" className="h1card">
            {item.name}
          </Card.Header>
          <Card.Body>
        ...
        {!qtityAdded ? (
          <ItemCount initial={1} stock={item.stock} onAdd={onAdd} item={item} />
        ) : (
          <Button variant="dark">
            {" "}
            <Link to="/cart" className="btn-color">
              Cart
            </Link>
          </Button>
        )}
